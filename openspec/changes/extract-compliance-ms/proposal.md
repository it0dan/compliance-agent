# Proposal: Extração do Microsserviço de Compliance Independente
...
2. **Ciclos de Evolução Desacoplados**: Alterações legislativas do BACEN, COAF ou diretrizes do DPO sobre a LGPD ocorrem em cronogramas independentes. O microsserviço precisa de deploy, testes de regressão e versionamento autônomos.
...
* Implementação do microsserviço em **TypeScript (strict mode)** com **Fastify** para o runtime HTTP.
...
* Comportamento determinístico simulado baseado nos blocos de CPF mascarado (sem acoplamento com LLM interna no microsserviço, que agora atua puramente como uma API de capability especializada).
...
O microsserviço de compliance atua de forma totalmente isolada em relação aos demais microsserviços do MAS de crédito. O único ponto de entrada para o orquestrador é o gateway da Sensedia.
...
| Característica | Mock no Core de Crédito (`compliance_agent.py`) | Novo Microsserviço Independente (`compliance-agent`) |
| :--- | :--- | :--- |
...
* **C-01:** Microsserviço compilando sem erros em modo TypeScript Estrito (`tsc`).
